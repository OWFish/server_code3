 ...
 *                        it uses char* instead of LPCSTR as first paremeter)
...
private:
  ...
  BOOL GetModuleListPSAPI(HANDLE hProcess)
  {
    ...
    // GetModuleFileNameEx()
    typedef DWORD (__stdcall *tGMFNE)(HANDLE hProcess, HMODULE hModule, char* lpFilename, DWORD nSize );
    // GetModuleBaseName()
    typedef DWORD (__stdcall *tGMBN)(HANDLE hProcess, HMODULE hModule, char* lpFilename, DWORD nSize );
    ...
    {
      //_ftprintf(fLogFile, ("%lu: EPM failed, GetLastError = %lu\n"), g_dwShowCount, gle );
      goto cleanup;
    }

    if ( cbNeeded > TTBUFLEN )
    {
      //_ftprintf(fLogFile, ("%lu: More than %lu module handles. Huh?\n"), g_dwShowCount, lenof( hMods ) );
      goto cleanup;
    }
...
{
	OnStackWalkStart();
	BOOL bRet = ShowCallStackImpl(hThread, context, readMemoryFunction, pUserData);
	OnStackWalkEnd();
	return bRet;
}

BOOL StackWalker::ShowCallStackImpl(HANDLE hThread, const CONTEXT *context, PReadProcessMemoryRoutine readMemoryFunction, LPVOID pUserData)
{
  CONTEXT c;;
    ...
    )
{
  if (s_readMemoryFunction == NULL)
  {
    ...
    *lpNumberOfBytesRead = (DWORD) st;
    //printf("ReadMemory: hProcess: %p, baseAddr: %p, buffer: %p, size: %d, read: %d, result: %d\n", hProcess, (void*) qwBaseAddress, lpBuffer, nSize, (DWORD) st, (DWORD) bRet);
    return bRet;
  }
  else
  {
