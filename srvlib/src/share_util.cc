...
/*
#ifndef _MSC_VER
	SecondTime now = SecondTime::now();
	SecondTime check = check.encode(2020, 12, 31, 0, 0, 0);
	if (now > check)
	{
		assert(false);
	}
...
bool CheckIdentity(const char* id, bool* enough18)
{
	...
	if (len == 15)
	{
		...
		while (str != endstr) if (*str < 48 || *str++ > 57) return false;
	...
	else if (len == 18)
	{
		...
		if (id[17] != 'x' && id[17] != 'X' && (id[17] < 48 || id[17] > 57)) return false;
		...
		if (id[17] != parity[s] && id[17] != parity_ex[s]) return false;
	}
	else
	{
		return false;
	...
	*enough18 = true;
	...
	if (delta_y > 18) return true;

	if (delta_y == 18)
	{
		int delta_m = tt->tm_mon + 1 - m;

		if (delta_m > 0) return true;

		if (delta_m == 0 && (tt->tm_mday >= d)) return true;
	}
