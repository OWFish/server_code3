...
// #define BSON_TIME_DEBUG

#define DEFAULT_CAP 65536 // 64
...
append_one(struct bson *bs, lua_State *L, const char *key, size_t sz) {
	...
	case LUA_TSTRING: {
		...
		if (len > 1 && str[0]==0) {
			...
			write_length(bs, (int32_t)len+1, off);		
		}
		break;
	}
	case LUA_TTABLE:
...
unpack_dict(lua_State *L, struct bson_reader *br, bool array) {
	...
	const uint8_t * bytes = (uint8_t *)read_bytes(L, br, sz - 5);
...
#ifdef BSON_TIME_DEBUG
	int64_t s = _getTickCount();
#endif // BSON_TIME_DEBUG
...
#ifdef BSON_TIME_DEBUG
	OutputMsg(rmTip, "bson decode time : %d", (int)(_getTickCount() - s));
#endif
...
#ifdef BSON_TIME_DEBUG
	OutputMsg(rmTip, "bson encode time : %d", (int)(_getTickCount() - s));
#endif
...
init_oid_header() {
	...
	oid_counter = (uint32_t)(h ^ time(NULL) ^ (uintptr_t)&h);
}
